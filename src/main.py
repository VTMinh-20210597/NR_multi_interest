...
def test(rank, args):
    ...
    for cnt, (log_vecs, log_mask, news_vecs, labels) in enumerate(dataloader):
        ...
        # print(f"user_vecs: {user_vecs.shape}") #user_vecs: torch.Size([128, 3, 400])

        for user_vec, news_vec, label in zip(user_vecs, news_vecs, labels):
            if label.mean() == 0 or label.mean() == 1:
                continue
            
            # print(f"user_vec: {user_vec.shape}") #user_vec: (3, 400)
            # print(f"news_vecs: {news_vec.shape}") #news_vecs: (7, 400)
            scores = np.dot(news_vec, user_vec.T)
